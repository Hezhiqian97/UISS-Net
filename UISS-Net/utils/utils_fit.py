    ...
    for iteration, batch in enumerate(gen_val):
        ...
        with torch.no_grad():
            ...
            if focal_loss:
                ...
                # s_loss = Focal_Loss(outputs, s_pngs, weights, num_classes=num_classes)
                s_loss=Dice_loss(outputs, labels)
                p_loss = CE_Loss(outputs, pngs, weights, num_classes=num_classes)
                # main_dice = Dice_loss(outputs, labels)
                a = 0.0001
                b = 0.001
                loss = p_loss #+s_loss # + a * s_loss  # +b*main_dice
